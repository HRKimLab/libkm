function hpol = polar09(varargin)
% CHANGE from polar(). to set better xlim, ylim. see CHANGE for modification
% xlim,ylim after drawing does not work for this type of patch-like grids.
...
% make a radial grid
    hold(cax,'on');
    maxrho = max([1 max(abs(rho(:)))]);
    hhh=line([-maxrho -maxrho maxrho maxrho],[-maxrho maxrho maxrho -maxrho],'parent',cax);
    set(cax,'dataaspectratio',[1 1 1],'plotboxaspectratiomode','auto')
    % CHANGE start. often axis gets too big, plot gets too small
    if abs(maxrho) < 1, maxrho == 2; end;
