function [grp_means grp_sem pRef pDiff pPair bar_res] = plot_barpair(x, is_sig, y_ref, varargin)
% plot bar plot with paired elements connected with lines
% 10/28/2017 HRK
% 2023 JWY - modified the color of each bar to be different and draw star by friedman test.
...
if ~is_arg('bar_color'), bar_color = [.5 .5 .5];    end;
...
[grp_means grp_sem pRef pDiff mc_sig ~,~,bar_res] = plot_bargrp_friedman(v, grp, 'is_sig', is_sig, 'y_ref', y_ref, 'yl', yl, ...
    'show_individual', show_individual, 'use_star', use_star, 'marker_color', bar_color, 'show_mc', 0, ...
...
if show_individual
    for iR = 1:size(x, 1)
        hSL = line(bar_x, x(iR,:),'color', bar_color);
...
switch(test_type)
    case 'nonpar'
        ...
        elseif nCond > 1
            ...
            pDiff_friedman = NaN(nCond, nCond);
            for iR = 1:nCond
                for iC = iR+1:nCond
                    if all(isnan(x(:,iR)) | isnan(x(:,iC))), continue; end
                    pDiff_friedman(iR, iC) = friedman([x(:,iR), x(:,iC)],1,'off');
                end
            end
...
if nCond == 2
    atitle(sprintf('p-signed=%s,p-friedman=%s', p2s(pPair(1,2)), p2s(pDiff(1,2))) );
else
...
for iR = 1:numel(unique(grp))
    for iC= (iR+1):(numel(unique(grp)))
        ...
        mc_sig2(i_row, 3) = pDiff(iR, iC) < 0.05;
        i_row = i_row + 1;
    end
end
...
if all(bar_x == (1:size(x, 2)) )
    switch show_mc
        ...
        case {2,3}
            ...
            for iR = 1:nCond
                for iC = iR+1:nCond
                    if show_mc == 2 || (show_mc == 3 && pDiff(iR, iC) < 0.05)
                        ss_grps = { ss_grps{:}, [iR iC]};
                        ss_stats = [ss_stats pDiff(iR, iC)];
                    end
                end
            end
